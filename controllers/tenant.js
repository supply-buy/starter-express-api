...
router.get('/gettenantList',async function (req, res, next) {
  let sqlQueryStr = "SELECT brand,name,account,email,mobile,taxid,type,status,created_at  FROM supplybuy.tenant"
  let rs =await client.execute( sqlQueryStr);
    if (!rs) {
      ...
      var dataList = rs.rows
      var resDataList = []
      dataList.forEach(element => {
        if (element.type === 'b')
          element.type = 'Buyer'
        else if (element.type == 's')
            element.type = 'Distributor'
        else if (element.type == 'brand')
            element.type = 'Brand'
        else if (element.type == 'badmin')
            element.type = 'Admin'
        
        resDataList.push(element)
      });
      res.send(resDataList)
    }
  });
